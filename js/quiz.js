  ...
  $.dlxQuiz = function (element, options) {
    var plugin = this,
      ...
      defaults = {
        /* Text
         ---------------------------*/
        questionCount_text: "Pregunta %current_index de %totalQuestions",
        backButton_text: "Anterior pregunta",
        nextButton_text: "Siguiente pregunta",
        completeButton_text: "Finalizar",
        viewResultsButton_text: "Ver resultados",
        resultsHeader_text: "Estos son tus resultados:",
        quizScore_text:
          "Respondiste correctamente %totalScore de un total de %totalQuestions preguntas",
        quizScoreMessage_text: "",
        quizScoreRank_text: {
          a: "¡Puntaje perfecto! 👏😁",
          b: "¡Buen trabajo! 👌😉",
          c: "¡Apenas y pasas! 😓:",
          d: "Debes de poner más atención 😥",
          f: "No lo estás intentando muy bien 😶",
        },
    ...
    plugin.method = {
      buildQuiz: function (data) {
        ...
        quizData = data;
        //set questions
        questions = plugin.config.randomizeQuestions
          ? plugin.method.randomizeArray(quizData.questions)
          : quizData.questions;
        //set question count
        ...
        $.each(questions, function (q) {
          ...
          question.options = plugin.config.randomizeAnswers
            ? plugin.method.randomizeArray(question.options)
            : question.options;
          ...
          if (plugin.config.show_QuestionCount) {
            _quizHTML += '<p class="' + class_questionCount + '">';
          ...
          $.each(question.options, function (a) {
            ...
            _quizHTML += "<input ";
            //show radio input
            _quizHTML += plugin.config.showRadioButtons
              ? ""
              : 'class="hidden" ';
      ...
      randomizeArray: function (array) {
        ...
        // While there remain elements to shuffleÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â¦
        while (m) {
          // Pick a remaining elementÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â¦
      ...
      buildQuizResults: function () {
        ...
        function _checkAnswers() {
          ...
          $.each(questions, function (index) {
            ...
            questions[index].answerCorrect =
              questions[index].selected === questions[index].a ? true : false;
        ...
        //quiz score rank
        resultsHTML += '<h4 class="' + class_quizScoreRank + '">';
        if (totalScore > 80) {
          //rank 1: 80-100
    ...
    plugin.events = {
      ...
      controls: {
        DOM: function () {
          ...
          //all buttons
          this.$buttons = this.$previous.add(this.$next).add(this.$complete);
        },
        init: function () {
        ...
        resetDisabled: function () {
          ...
          this.$next =
            questions[question_index].selected !== undefined
      ...
      resultsButton: function () {
        $(_quizViewResultsButton).on("click", function () {
          ...
          //build results
          resultsHTML =
            '<p class="resultsHeader_text">' +
            plugin.config.resultsHeader_text +
            "</p>";
          ...
          $.each(questions, function (index) {
            ...
            //question
            resultsHTML += '<p class="questionTitle">';
            ...
            if (!questions[index].answerCorrect) {
              //display user answer
              resultsHTML += "<strong>Tu respuesta: </strong>";
              resultsHTML += questions[index].selected + "<br>";
            }
            //display correct answer
            resultsHTML += "<strong>Respuesta correcta: </strong>";
